...
The root `AGENTS.md` is the repository's normative control plane. Its rules apply before deeper lookup. Sweet Cookie is an upstream-derived browser-cookie library and extension workspace maintained under Hraness without changing its public `@steipete/sweet-cookie` identity.
