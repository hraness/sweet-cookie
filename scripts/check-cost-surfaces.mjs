...
for (const [id, entry] of Object.entries(surfaces)) {
  ...
  if (typeof entry.retention !== "string" || !RETENTION.test(entry.retention)) {
    fail(
      `${id}: retention must be ephemeral|ttl:<ISO8601>|account|tombstone|persistent`,
    );
  }
  ...
  if (entry.kind === "authoritative") {
    if (
      typeof entry.deletion !== "string" ||
      !existsSync(join(root, entry.deletion))
    ) {
  ...
  if (
    ...
    typeof entry.maxBytesPerEvent !== "number"
  ) {
  ...
  if (entry.kind === "derived" && typeof entry.source !== "string") {
    fail(
      `${id}: derived surfaces need a source naming their authoritative origin`,
    );
  }
}
...
for (const schemaPath of backendSchemas()) {
  ...
  for (const name of tableNames) {
    ...
    if (!surfaces[id] && !exempt.has(id)) {
      fail(
        `unregistered backend table "${name}" (${relative(root, schemaPath)}) — add "${id}" to costs.json`,
      );
    }
  }
}
...
for (const ev of seenEvents) {
  ...
  if (!surfaces[id] && !exempt.has(id)) {
    fail(
      `unregistered analytics event "${name}" in ${rel} — add "${id}" to costs.json`,
    );
  }
}
...
for (const r of seenRoutes) {
  ...
  if (!surfaces[id] && !exempt.has(id)) {
    fail(
      `dynamic/edge route ${path || "/"} (${rel}) is unregistered — add "${id}" to costs.json`,
    );
  }
}
